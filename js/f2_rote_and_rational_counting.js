        ...
        CrossPlatformDragManager: {
            ...
            setupHTML5DragSystem(difficulty, config) {
                ...
                try {
                    ...
                    console.warn('⚠️ HTML5拖曳系統初始化失敗，遊戲仍可正常運行（主要為點擊模式）');
                }
            },
        ...
        HTML5DragSystem: {
            ...
            initialize(difficulty) {
                if (this.isInitialized) {
                    console.warn('🔄 F2 HTML5 拖曳系統已初始化，跳過重複初始化');
                    return;
                }

                const itemArea = document.getElementById('item-area');
                if (!itemArea) {
                    console.error('❌ 找不到 item-area 元素，跳過拖曳設置');
                    return;
                }

                console.log('🎯 初始化 F2 HTML5 拖曳系統', { difficulty });
                ...
                this.isInitialized = true;
                console.log('✅ F2 HTML5 拖曳系統初始化完成');
            },
            ...
            setupTouchDragSupport(container) {
                ...
                try {
                    ...
                    console.log('✅ TouchDragUtility 註冊完成');
                } catch (error) {
                    console.error('❌ TouchDragUtility 註冊失敗:', error);
                }
            },
            ...
            cleanup() {
                ...
                // 取消 TouchDragUtility 註冊
                if (window.TouchDragUtility && itemArea) {
                    window.TouchDragUtility.unregisterDraggable(itemArea);
                    window.TouchDragUtility.unregisterDropZone(itemArea);
                }

                this.isInitialized = false;
                console.log('✅ F2 HTML5 拖曳系統清理完成');
            }
        }
    };
