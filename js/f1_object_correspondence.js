...
document.addEventListener('DOMContentLoaded', () => {
    Game = {
        ...
        ModeConfig: {
            easy: {
                ...
                speechTemplates: {
                    ...
                    initialInstruction: "請拖曳放置一樣的物品數量，您也可以點擊物品來放置",
            ...
            normal: {
                ...
                // 語音模板配置
                speechTemplates: {
                    initialInstruction: "請將 {targetCount} 個物品拖曳到對應框的放置區，您也可以點擊物品來放置，完成後按下完成按鈕",
            ...
            hard: {
                ...
                // 語音模板配置
                speechTemplates: {
                    initialInstruction: "請觀察對應框中的目標，將物品拖曳到放置區，您也可以點擊物品來放置。",
                    correct: "答對了，太棒了！",
        ...
        // =====================================================
        gameData: {
            // 固定版本號 - 最後修正時間：2025.08.29 02:58
            title: `單元F1：物件對應 (v2025.08.29 02:58)`,
            themes: {
                fruits:  ['🍎', '🍌', '🍇', '🍓', '🍊', '🥝', '🍍', '🍉', '🍑', '🍒'],
                animals: ['🐶', '🐱', '🐭', '🐰', '🦊', '🐻', '🐼', '🐨', '🐯', '🦁'],
                vehicles:['🚗', '🚕', '🚌', '🚓', '🚑', '🚒', '🚚', '🚲', '🚀', '✈️'],
        ...
        showSettings() {
            ...
            app.querySelector('#start-game-btn').addEventListener('click', this.startGame.bind(this));
        ...
        async start() {
            ...
            Game.Debug.logConfig('載入' + this.state.settings.difficulty + '模式配置', this.ModeConfig[this.state.settings.difficulty]);
        ...
        setupGameUI() {
            ...
            const { theme, questionCount, testMode, countingRange } = this.state.settings;
            
            // 載入遊戲佈局模板
            const app = document.getElementById('app');
            app.innerHTML = this.HTMLTemplates.gameLayout(difficulty, questionCount, '請拖曳正確數量的物品到對應框中');
            
            // 綁定事件
            this.bindEvents(difficulty);
        },

        bindEvents(difficulty) {
            Game.Debug.logUI('取得DOM元素', undefined);
            
            // F1使用拖拽系統，跳過點擊事件綁定
            Game.Debug.logUI('F1使用拖拽系統，跳過點擊事件綁定', undefined);
            
            // 🚀 初始化跨平台拖曳管理器
            Game.Debug.logUI('🚀 初始化跨平台拖曳管理器 (HTML5)', difficulty);
            
            try {
                Game.HTML5DragSystem.initialize(difficulty);
                console.log('✅ F1 HTML5拖曳系統初始化成功');
            } catch (error) {
                console.error('❌ F1 HTML5拖曳系統初始化失敗:', error);
            }
        ...
        startNewTurn() {
            ...
            Game.Debug.logState('新回合狀態', this.state);
            
            // 清空遊戲區域
            Game.Debug.logUI('清空遊戲區域', undefined);
            
            // 生成新題目
            const question = this.generateNewQuestion();
            Game.Debug.logGameFlow('生成新題目', question);
            
            // 渲染模式特定UI
            this.renderModeSpecificUI(question.primaryIcon, this.state.settings.difficulty);
            Game.Debug.logGameFlow('renderModeSpecificUI完成');
            
            // 播放初始指示語音
            this.Speech.speak('initialInstruction', this.state.settings.difficulty, 
                this.ModeConfig[this.state.settings.difficulty], {
        ...
        renderModeSpecificUI(primaryIcon, difficulty) {
            // 簡化的UI渲染邏輯
            Game.Debug.logGameFlow('執行' + difficulty + '模式渲染');
        },
        ...
        HTML5DragSystem: {
            ...
            initialize(difficulty) {
                if (this.isInitialized) {
                    console.warn('🔄 F1 HTML5 拖曳系統已初始化，跳過重複初始化');
                    return;
                }

                console.log('🎯 初始化 F1 HTML5 拖曳系統', { difficulty });
                ...
                console.log('✅ F1 HTML5 拖曳系統初始化完成');
            },
             ...
             */
            setupDragSources() {
                const sourceItems = document.querySelectorAll('.money-item, .draggable-item');
                ...
                sourceItems.forEach(item => {
                    if (!item.hasAttribute('draggable')) {
                        ...
                        console.log('✅ 設置拖曳源項目', { id: item.dataset.id, className: item.className });
                    }
                });
            },
             ...
             */
            setupDropTargets() {
                const dropZones = document.querySelectorAll('.drop-zone, .normal-grid-item, .placement-grid .target-grid-item');
                ...
                dropZones.forEach(zone => {
                    ...
                    console.log('✅ 設置放置區域', { className: zone.className });
                });
            },
            ...
            handleDragStart(event) {
                const element = event.target.closest('.money-item, .draggable-item');
                if (!element) return;

                this.dragState.dragElement = element;
                this.dragState.startContainer = element.parentNode;
                this.dragState.originalParent = element.parentNode;

                // 設置拖曳資料
                event.dataTransfer.effectAllowed = 'move';
                event.dataTransfer.setData('text/plain', element.dataset.id || element.dataset.index);
                ...
                console.log('🎯 開始拖曳', { 
                    ...
                    from: this.dragState.startContainer.className 
                });
            },
            ...
            handleDragEnd(event) {
                ...
                // 清除所有放置區域的視覺提示
                document.querySelectorAll('.drop-zone, .normal-grid-item, .target-grid-item').forEach(zone => {
            ...
            handleDragEnter(event) {
                const zone = event.target.closest('.drop-zone, .normal-grid-item, .target-grid-item');
                if (!zone) return;

                // 檢查是否已經有項目
                const existingItems = zone.querySelectorAll('.money-item, .draggable-item');
            ...
            handleDrop(event) {
                ...
                const zone = event.target.closest('.drop-zone, .normal-grid-item, .target-grid-item');
                if (!zone || !this.dragState.dragElement) return;
                ...
                console.log('✅ 成功放置項目', {
                    id: this.dragState.dragElement.dataset.id,
                    to: zone.className
                });
                ...
                switch(config.modeType) {
                    case 'one-to-one-correspondence':
                        this.handleEasyDrop(event, zone, this.dragState.dragElement, difficulty, config);
                        break;
                    case 'quantity-to-numeral':
                    case 'multi-item-correspondence':
                        // 普通和困難模式使用原本的處理方式
                        zone.appendChild(this.dragState.dragElement);
                        this.hidePositionHints(zone);
                        if (typeof Game.handleDragEnd === 'function') {
                            Game.handleDragEnd({ target: this.dragState.dragElement });
                        }
                        break;
                }
            },
            ...
            /**
             * 處理簡單模式的放置邏輯（包含配對動畫和自動進入下一題）
             */
            handleEasyDrop(event, dropZone, draggedElement, difficulty, config) {
                if (!dropZone.classList.contains('drop-zone')) {
                    console.log('❌ 不是有效的簡單模式放置區域');
                    return;
                }
                ...
                if (placedItem) {
                    ...
                    placedItem.style.pointerEvents = 'none';
                }
                ...
                // 檢查是否完成當前回合
                if (Game.state.correctPlacements >= Game.state.correctAnswer) {
             ...
             * 隱藏位置提示文字
             */
            hidePositionHints(targetZone) {
                setTimeout(() => {
                    if (targetZone) {
                        const hints = targetZone.querySelectorAll('.placement-hint, .hint-text, .hint-number, .question-hint');
                        hints.forEach(hint => {
                            hint.style.display = 'none';
                            console.log('✅ 提示文字已隱藏', { 
                                hintClass: hint.className,
                                targetZone: targetZone.className
                            });
                        });
                    }
                }, 0);
             ...
             * 顯示位置提示文字
             */
            showPositionHints(targetZone) {
                if (targetZone) {
                    const hints = targetZone.querySelectorAll('.placement-hint, .hint-text, .hint-number, .question-hint');
                    hints.forEach(hint => {
                        hint.style.display = '';
                        console.log('✅ 恢復提示文字顯示', { 
                            hintClass: hint.className,
                            targetZone: targetZone.className
                        });
                    });
            ...
            /**
             * 設置 TouchDragUtility 支援
             */
            setupTouchDragSupport() {
                if (typeof window.TouchDragUtility !== 'undefined') {
                    console.log('🤚 啟用 TouchDragUtility 支援');
                    // TouchDragUtility 會自動處理觸控事件
                } else {
                    console.log('ℹ️ TouchDragUtility 不可用，僅使用原生 HTML5 拖曳');
                }
            },
             ...
             */
            cleanup() {
                console.log('🧹 清理 F1 HTML5 拖曳系統');

                // 移除所有事件監聽器
                document.querySelectorAll('.money-item, .draggable-item').forEach(item => {
        ...
        setupGameUI() {
            ...
            app.addEventListener('click', (event) => {
                ...
                // 檢查點擊到放置區域（用於點擊放置功能）
                const dropZone = event.target.closest('.drop-zone, .placement-zone, .quantity-container, .normal-grid-item, .target-grid-item');
        ...
        renderNormalMode(icon, difficulty) {
            ...
            styleTag.innerHTML = `
                ...
                .placement-grid-normal .normal-grid-item {
                    ...
                    width: 80px !important; /* 固定寬度防止壓縮 */
                    height: 80px !important; /* 固定高度防止壓縮 */
                    padding: 8px;
                    border-radius: 8px;
                    ...
                    box-sizing: border-box;
                    position: relative;
                    transition: border-color 0.3s ease, background-color 0.3s ease; /* 只允許顏色過渡 */
                    flex-shrink: 0 !important; /* 禁止收縮 */
                    flex-grow: 0 !important; /* 禁止放大 */
                }
                
                /* 普通模式拖曳懸停效果 - 不改變尺寸 */
                .placement-grid-normal .normal-grid-item.drag-over,
                .placement-grid-normal .normal-grid-item.sortable-drag-over {
                    border-color: #4caf50;
                    background: rgba(76, 175, 80, 0.1);
                    transform: none !important; /* 禁止任何變形 */
                }
                
                /* 普通模式位置編號和已填充樣式 */
                .placement-grid-normal .position-hint { font-size: 0.9em; color: #999; }
                .placement-grid-normal .normal-grid-item.filled .position-hint { display: none !important; }
                .placement-grid-normal .normal-grid-item.filled { 
                    border: 2px solid #ff9800; 
                    background: white; /* 外層方框白色背景 */
                    position: relative !important; /* 為絕對定位提供參考點 */
                }
                .placement-grid-normal .normal-grid-item.filled .draggable-item { 
                    background: #e8f5e8; /* 內層方框綠色背景 */
                    border: 1px solid #4caf50;
                    /* 🔧 [修復] 絕對定位覆蓋整個放置框，避免與文字擠壓 */
                ...
                /* 目標區內層樣式 - 與對應框的 placement-grid-normal 一致，改為水平排列 */
        ...
        renderHardMode(primaryIcon, difficulty) {
            ...
            styleTag.innerHTML = `
                ...
                /* 讓放置區和目標區的 Grid 共享水平排列樣式 */
                .target-grid, .placement-grid {
                    display: flex; /* 修改為 flex 實現水平排列 */
                ...
                .placement-grid .target-grid-item.filled .position-hint { display: none !important; }
        ...
        handleDragStart(event) {
            ...
            // SortableJS可能不提供dataTransfer對象，添加安全檢查
        ...
        handleDrop(event) {
            ...
            const draggedId = event.dataTransfer ? event.dataTransfer.getData('text/plain') : null;
            if (!draggedId) {
        ...
        handleNormalDrop(event, draggedElement, difficulty, config) {
            Game.Debug.groupStart('handleNormalDrop 執行'); // 開始日誌分組
        ...
        handleZoneClick(event) {
            ...
            // 找到點擊的放置區域
            let dropZone = event.target.closest('.drop-zone, .placement-zone, .quantity-container, .normal-grid-item, .target-grid-item');
        ...
        HTML5DragSystem: {
            ...
            initialize(difficulty) {
                if (this.isInitialized) {
                    console.warn('🔄 F1 HTML5 拖曳系統已初始化，執行重新初始化');
                    this.cleanup();
                }
                ...
                // 設置來源區域
                this.setupSourceArea();
                
                // 設置放置區域（根據不同難度）
                this.setupDropAreas(difficulty);
            ...
            setupDropAreas(difficulty) {
                ...
                switch (difficulty) {
                    case 'easy':
                        ...
                        break;
                    case 'normal':
                        dropSelectors = ['.normal-grid-item', '.placement-zone'];
                        break;
                    case 'hard':
                        dropSelectors = ['.target-grid-item', '.placement-zone'];
                        break;
                }
                ...
                console.log('✅ 放置區域設置完成', { difficulty, zones: dropSelectors });
            },
            ...
            setupTouchDragSupport() {
                if (!window.TouchDragUtility) {
                    console.warn('⚠️ TouchDragUtility 未載入，跳過手機拖曳支援');
                    return;
                }
                ...
                try {
                    ...
                    if (sourceContainer) {
                        window.TouchDragUtility.registerDraggable(
                            ...
                            {
                                onDragStart: this.handleTouchDragStart.bind(this),
                                onDrop: this.handleTouchDrop.bind(this)
                            }
                        );
                    }

                    // 註冊放置區域
                    const dropZones = document.querySelectorAll('.drop-zone, .normal-grid-item, .target-grid-item, .placement-zone');
                    dropZones.forEach(zone => {
                        window.TouchDragUtility.registerDropZone(zone, (dragElement, dropZone) => {
                            return this.validateDrop(dragElement, dropZone);
                        });
                    });

                    console.log('✅ TouchDragUtility 手機支援設置完成');
                } catch (error) {
                    console.error('❌ TouchDragUtility 設置失敗:', error);
                }
            },
            ...
            handleDrop(event) {
                ...
                if (!dragElement || !this.validateDrop(dragElement, dropZone)) {
                    return;
                }
            ...
            handleTouchDragStart(element, event) {
                ...
                console.log('📱 手機拖曳開始:', element.dataset.id || element.dataset.icon);
                return true; // 允許拖曳
            },
            ...
            executeDrop(dragElement, dropZone) {
                ...
                console.log('🎯 執行放置操作:', {
                    element: dragElement.dataset.id || dragElement.dataset.icon,
                    from: originalParent.className,
                    to: dropZone.className,
                    difficulty
                });

                // 處理從放置區域拖回來源區域
                if (dropZone.classList.contains('source-container')) {
                    this.handleReturnToSource(dragElement, dropZone, originalParent, difficulty);
                    return;
                }

                // 處理放置到目標區域
                this.handlePlaceToTarget(dragElement, dropZone, originalParent, difficulty);
            },
            ...
            cleanup() {
                ...
                // 清理 TouchDragUtility（如果有的話）
                if (window.TouchDragUtility && window.TouchDragUtility.cleanup) {
                    try {
                        window.TouchDragUtility.cleanup();
                    } catch (error) {
                        console.warn('⚠️ TouchDragUtility 清理警告:', error);
                    }
                }
