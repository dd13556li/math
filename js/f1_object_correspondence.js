        ...
        TouchDragManager: {
            ...
            init(difficulty, config) {
                ...
                const checkUtilityReady = () => {
                    ...
                    // 詳細檢查 TouchDragUtility 物件
                    Game.Debug.logUI('檢查 TouchDragUtility 狀態', `第 ${attempts} 次嘗試`, {
                        windowTouchDragUtility: typeof window.TouchDragUtility,
                        windowKeys: Object.keys(window).filter(key => key.includes('Touch')),
                        touchUtilityExists: 'TouchDragUtility' in window
                    });
                    
                    // 檢查 TouchDragUtility 是否已成功載入並掛載到 window 物件上
                    if (typeof window.TouchDragUtility !== 'undefined' && window.TouchDragUtility !== null) {
                        Game.Debug.logUI('TouchDragUtility 已準備就緒，開始初始化', `嘗試次數: ${attempts}`, {
                            methods: Object.getOwnPropertyNames(window.TouchDragUtility).filter(name => typeof window.TouchDragUtility[name] === 'function')
                        });
                        ...
                        setTimeout(() => {
                            try {
                                ...
                                Game.Debug.logError(error, '觸控拖曳註冊失敗');
                        ...
                        // 如果超過最大嘗試次數後仍然找不到，則記錄錯誤和除錯資訊
                        Game.Debug.logError('TouchDragUtility 載入超時或失敗', 'TouchDragManager.init', {
                            attempts: attempts,
                            windowTouchDragUtility: typeof window.TouchDragUtility,
                            allWindowProps: Object.getOwnPropertyNames(window).filter(prop => prop.toLowerCase().includes('touch'))
                        });
                    }
                };
            ...
            createMockDropEvent(draggedElement, dropZone, syntheticEvent) {
                return {
                    ...
                    dataTransfer: {
                        getData: (type) => {
                            if (type === 'text/plain') {
                                return draggedElement.dataset.id || draggedElement.dataset.index || draggedElement.dataset.value || '';
                            }
                            if (type === 'icon') {
            ...
            cleanup() {
                if (this.isInitialized && typeof window.TouchDragUtility !== 'undefined') {
                    try {
                        // 清理所有註冊的處理器
                        const app = document.getElementById('app');
                        if (app && window.TouchDragUtility.unregisterDraggable) {
                            window.TouchDragUtility.unregisterDraggable(app);
                        }
                        
                        if (this.currentConfig?.selectors?.dropZone && window.TouchDragUtility.unregisterDropZone) {
                        ...
                        Game.Debug.logError(error, '觸控拖曳清理時發生錯誤');
                        // 強制重置狀態
                        this.isInitialized = false;
                        this.currentConfig = null;
                    }
