...
document.addEventListener('DOMContentLoaded', () => {
    Game = {
        ...
        CrossPlatformDragManager: {
                ...
                if (sourceContainer) {
                    const sourceInstance = new Sortable(sourceContainer, {
                        group: {
                            ...
                            pull: true,
                            put: true // 【核心修正】允許從其他容器（如放置框）將物品放回來
                        },
                        sort: false, // 來源區域不允許排序
                        animation: 150,
                        
                        // 🔧 [智能拖曳配置] 
                        forceFallback: false, // 先嘗試原生拖放
                        fallbackClass: 'sortable-fallback',
                        fallbackOnBody: true, // 拖曳元素附加到 body
                        fallbackAxis: undefined, // 允許全方向移動
                        fallbackOffset: { x: 0, y: 0 }, // 偏移設為 0 確保跟手
                        
                        // 觸控優化配置
                        delayOnTouchStart: true, 
                        delay: 50, // 減少延遲提高響應性
                        touchStartThreshold: 3, // 提高觸控靈敏度
                        swapThreshold: 0.5, // 降低交換閾值提高精確度
                        
                        onStart: (evt) => {
                            Game.Debug.logUserAction('🎯 SortableJS 拖曳開始', {
                                element: evt.item.dataset,
                                from: evt.from.className,
                                fallbackMode: evt.fallback ? '✅ Fallback啟用' : '❌ 原生拖曳'
                            });
                            
                            // 🔧 [調試] 檢查 fallback 狀態
                            if (!evt.fallback) {
                                Game.Debug.logUserAction('⚠️ 警告：未進入fallback模式，拖曳可能不跟手');
                            }
                            
                            // 🔧 [修復] 直接傳遞正確的元素給拖曳邏輯
                            // 跳過 handleDragStart，直接執行必要的邏輯
                            const element = evt.item;
                            const draggedId = element.dataset.id || element.dataset.index;
                            const icon = element.dataset.icon;
                            
                            Game.Debug.logUserAction('✅ SortableJS 拖曳元素確認', {
                        ...
                        onEnd: (evt) => {
                            Game.Debug.logUserAction('🏁 SortableJS 拖曳結束', {
                                element: evt.item.dataset,
                                from: evt.from.className,
                                to: evt.to?.className
                            });
                            ...
                            document.querySelectorAll('.placement-zone').forEach(zone => {
                                zone.classList.remove('sortable-drag-over');
                            });
                            
                            // 調用原有的拖曳結束邏輯
                            const mockEvent = { target: evt.item };
                            Game.handleDragEnd(mockEvent);
                        }
                    });

                    this.sortableInstances.push(sourceInstance);
                    Game.Debug.logUI('✅ 來源區域 Sortable 實例已創建');
                }

                // 為放置區域設置拖曳 (包含簡單、普通、困難模式的所有可放置格子)
                const placementSlots = document.querySelectorAll('.drop-zone, .normal-grid-item, .placement-grid .target-grid-item');
                placementSlots.forEach((zone, index) => {
                    const zoneInstance = new Sortable(zone, {
                        group: {
                            name: 'correspondence-game',
                            pull: true,
                            put: true
                        ...
                        onMove: (evt) => {
                            ...
                            // 檢查目標格子是否已滿 (對簡單、普通和困難模式都有效)
                            if (targetZone && (targetZone.classList.contains('drop-zone') || targetZone.classList.contains('normal-grid-item') || targetZone.classList.contains('target-grid-item'))) {
                                const existingItems = targetZone.querySelectorAll('.draggable-item');
                                if (existingItems.length >= 1) {
                                    Game.Debug.logUserAction('❌ 放置區域已滿，禁止放置', {
                                        zone: targetZone.className,
                                        existingItems: existingItems.length
                                    });
                                    return false; // 【關鍵】返回 false 來禁止移動
                                }
                            }
                            
                            // 允許放置，添加視覺效果
                            if (targetZone) {
                                targetZone.classList.add('sortable-drag-over');
                                
                                // 移除其他區域的視覺效果
                                document.querySelectorAll('.drop-zone, .normal-grid-item, .target-grid-item').forEach(z => {
                                    if (z !== targetZone) {
                                        z.classList.remove('sortable-drag-over');
                                    }
                                });
                            }
                        ...
                        onAdd: (evt) => {
                            Game.Debug.logUserAction('📥 SortableJS 元素放置', {
                                element: evt.item.dataset,
                                to: evt.to.className,
                            ...
                            // 🔧 [雙重檢查] 確認目標格子未滿（修正SortableJS的onMove限制失效問題）
                            const targetZone = evt.to;
                            const existingItems = targetZone.querySelectorAll('.draggable-item');
                            
                            if (existingItems.length > 1) {
                                // 如果格子已經有超過1個物件，強制移除新放置的物件並返回來源
                                Game.Debug.logUserAction('🚫 格子已滿，強制移除重複放置的物件', {
                                    zone: targetZone.className,
                                    existingItems: existingItems.length,
                                    action: 'force-remove'
                                });
                                ...
                                // 播放錯誤音效
                                Game.Audio.playSound('error', Game.state.settings.difficulty, Game.ModeConfig[Game.state.settings.difficulty]);
                                return;
                            }
                            
                            // 正常處理放置邏輯
                            if (Game.state.settings.difficulty === 'easy') {
                                this.handleEasyPlacement(evt);
                            } else if (Game.state.settings.difficulty === 'normal') {
                                this.handleNormalPlacement(evt);
                            } 
                            // 【新增困難模式處理】
                            else if (Game.state.settings.difficulty === 'hard') {
                                this.handleHardPlacement(evt);
                            }
                        },

                        onRemove: (evt) => {
                            Game.Debug.logUserAction('📤 SortableJS 元素移除', {
                                element: evt.item.dataset,
                                from: evt.from.className
                            });
                            
                            if (Game.state.settings.difficulty === 'normal') {
                                this.handleNormalReturn(evt);
                            } 
                            // 【新增困難模式處理】
                            else if (Game.state.settings.difficulty === 'hard') {
                                this.handleHardReturn(evt);
                            }
                        }
                    });

                    this.sortableInstances.push(zoneInstance);
                });

                Game.Debug.logUI('✅ 所有放置區域 Sortable 實例已創建', { count: placementSlots.length });
            },

            // 處理簡單模式放置
            handleEasyPlacement(evt) {
            ...
            // 處理普通模式放置
            handleNormalPlacement(evt) {
                const targetSlot = evt.to;
                targetSlot.classList.add('filled');
                Game.Audio.playSound('correct', 'normal', Game.ModeConfig.normal);
            },
            
            // 處理普通模式從格子取回
            handleNormalReturn(evt) {
                const originalSlot = evt.from;
                originalSlot.classList.remove('filled');
                originalSlot.innerHTML = `<span class="position-hint">${parseInt(originalSlot.dataset.index) + 1}</span>`;
                Game.Audio.playSound('select', 'normal', Game.ModeConfig.normal);
            },

            // 【新增】處理困難模式放置
            handleHardPlacement(evt) {
                const targetSlot = evt.to;
                targetSlot.classList.add('filled');
                Game.Audio.playSound('correct', 'hard', Game.ModeConfig.hard);
            },

            // 【新增】處理困難模式從格子取回
            handleHardReturn(evt) {
                const originalSlot = evt.from;
                originalSlot.classList.remove('filled');
                originalSlot.innerHTML = `<span class="position-hint">${parseInt(originalSlot.dataset.index) + 1}</span>`;
                Game.Audio.playSound('select', 'hard', Game.ModeConfig.hard);
            },
            ...
            cleanup() {
                if (this.sortableInstances.length > 0) {
                    try {
                        ...
                        Game.Debug.logUI('🧹 SortableJS 實例清理完成');
                    } catch (error) {
                        Game.Debug.logError(error, 'SortableJS 清理時發生錯誤');
                        // 強制重置
                        this.sortableInstances = [];
                    }
                }
        ...
        renderNormalMode(icon, difficulty) {
            ...
            if (touchConfig.touchDragConfig?.enabled) {
                setTimeout(() => {
                    this.CrossPlatformDragManager.cleanup();
                    this.CrossPlatformDragManager.init(difficulty, touchConfig);
