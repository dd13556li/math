    ...
    Game = {
        ...
        ModeConfig: {
            easy: {
                ...
                clickToMoveConfig: {
                    ...
                    speechFeedback: true,             // 點擊時語音回饋（播放金額）
        ...
        ModeStrategies: {
            ...
            setupTouchDragForMode(gameArea, mode, handlers, config, autoTrigger = false) {
                console.log('🎯 [C3-錢幣兌換] 檢查 TouchDragUtility 狀態', {
                ...
                if (!window.TouchDragUtility) {
                    console.error('❌ [C3-錢幣兌換] TouchDragUtility 未載入，觸控拖曳功能無法使用');
                    return;
                }
                
                console.log('✅ [C3-錢幣兌換] TouchDragUtility 已載入，開始註冊觸控拖曳');
                                ...
                                dataTransfer: {
                                    getData: (type) => {
                                        ...
                                        if (type === 'text/plain') {
                                            ...
                                            console.log('🎯 [C3-觸控拖拽] 取得coinId:', coinId, 'from element:', draggedElement);
                                            return coinId || draggedElement.dataset.value || '';
                                        }
                                        return draggedElement.dataset.value || '';
                                    }
                                },
        ...
        setupDragDropEvents(question, difficulty) {
            ...
            if (config?.clickToMoveConfig?.enabled) {
                console.log('🎯 [C3點擊除錯] 設置點擊事件處理', { difficulty });
                ...
                this._clickEventHandler = (event) => {
                    ...
                    if (moneyItem) {
                        console.log('✅ [C3點擊除錯] 發現金錢物品點擊，路由到 handleActionClick');
                ...
                console.log('✅ [C3點擊除錯] 點擊事件已成功綁定到 #dynamic-game-container');
            }
        ...
        handleActionClick(event) {
            ...
            console.log('🎯 [C3點擊除錯] handleActionClick 被呼叫', {
                target: target,
            ...
            if (target.id === 'complete-exchange-btn') {
                ...
                console.log('✅ [C3點擊除錯] 找到金錢物品，路由到點擊移動邏輯');
                this.handleItemClick(event);
            } else {
                console.log('❌ [C3點擊除錯] 目標不是有效的可點擊金錢物品');
            }
        },
        ...
        handleItemClick(event) {
            ...
            if (!config?.clickToMoveConfig?.enabled) {
                console.log('⚠️ 點擊移動功能未啟用');
                return;
            }
            
            console.log('🎯 [C3點擊除錯] handleItemClick 被呼叫', { difficulty });
            ...
            if (!clickedElement) {
                console.log('❌ [C3點擊除錯] 未找到有效的金錢物品元素');
                return;
            }
            
            console.log('✅ [C3點擊除錯] 找到可點擊的金錢物品', {
        ...
        handleClickToPlace(sourceItem, event) {
            ...
            console.log('🎯 [C3點擊除錯] handleClickToPlace 被呼叫', {
                sourceItem: sourceItem,
                difficulty: difficulty
            });
            
            if (!config?.clickToMoveConfig?.allowClickToPlace) {
                console.log('❌ 此模式不允許點擊放置');
                return;
            }
            ...
            if (isSameElement && isWithinDoubleClickTime) {
                // 這是第二次點擊（雙擊），執行放置
                console.log('✅ [C3點擊除錯] 偵測到雙擊，準備執行放置');
                ...
                console.log('🔄 [C3點擊除錯] 執行物品放置邏輯');
                
                // 執行放置邏輯 - 模擬拖拽放置
                this.simulateCoinPlacement(sourceItem);
                
                console.log('✅ [C3點擊除錯] 雙擊放置執行完成');
            } else {
                // 這是第一次點擊，僅選擇物品
                console.log('🔵 [C3點擊除錯] 第一次點擊，選擇物品');
        ...
        handleClickToReturn(placedItem, event) {
            ...
            console.log('🔙 [C3點擊除錯] 處理點擊取回', { placedItem });
        ...
        simulateCoinReturn(placedItem) {
            ...
            if (myMoneyArea) {
                console.log('📍 [C3點擊除錯] 找到金錢區域，直接呼叫processReturnToMoneyArea');
                ...
                console.log('✅ [C3點擊除錯] 取回邏輯執行完成');
            } else {
                console.log('❌ [C3點擊除錯] 找不到金錢區域');
            }
        },
