...
document.addEventListener('DOMContentLoaded', () => {
    const Game = {
        ...
        showShoppingScene() {
            ...
            // 🎯 延遲初始化TouchDragUtility，確保DOM已完全載入
            setTimeout(() => {
        ...
        initializeTouchDragSupport() {
            if (!window.TouchDragUtility) {
                console.warn('⚠️ TouchDragUtility 未載入，跳過手機拖放支援');
                return;
            }

            try {
                console.log('🎯 [A1] 初始化 TouchDragUtility 手機拖放支援');

                // 先清理之前的註冊
                const gameArea = document.getElementById('app');
                if (!gameArea) {
                    console.error('❌ 找不到遊戲區域，跳過TouchDragUtility註冊');
                    return;
                }
                ...
                window.TouchDragUtility.registerDraggable(
                    gameArea,
                    '.money-item[draggable="true"], .payment-money-item[draggable="true"], .change-money[draggable="true"], .normal-change-money[draggable="true"]',
                    {
                        onDragStart: (element, event) => {
                            console.log('📱 TouchDrag 金錢拖拽開始:', element.dataset.moneyId);
                            return true; // 允許拖拽
                        },
                        onDrop: (element, dropZone, event) => {
                            console.log('📱 TouchDrag 金錢放置:', element.dataset.moneyId, dropZone.className);
                            ...
                            const difficulty = this.state.settings.difficulty;
                            if (difficulty === 'easy') {
                                this.handleHintItemDrop(syntheticEvent);
                            } else {
                                this.handlePaymentZoneDrop(syntheticEvent);
                            }
                        },
                        onDragEnd: (element, event) => {
                            console.log('📱 TouchDrag 拖拽結束');
                        }
                    }
                );
                ...
                console.log(`✅ A1 TouchDragUtility 註冊完成: ${dropZones.length}個放置區域`);
                console.log(`🔍 放置區域詳情:`, dropZones.map(zone => zone.className));

            } catch (error) {
                console.error('❌ A1 TouchDragUtility 初始化失敗:', error);
            }
        },
