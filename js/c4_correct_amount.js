    ...
    Game = {
        ...
        setupTouchDragForEasyMode(question) {
            ...
            const registerTouchDrag = () => {
                ...
                if (sourceItems.length === 0) {
                    console.log('⏰ [簡單模式] 等待元素生成，延遲重試TouchDragUtility註冊...');
                    setTimeout(registerTouchDrag, 100);
                    return;
                }
                
                console.log(`✅ [簡單模式] 找到 ${sourceItems.length} 個可拖拽元素，開始註冊TouchDragUtility`);
        ...
        setupTouchDragForNormalMode(question) {
            ...
            const registerTouchDrag = () => {
                ...
                if (sourceItems.length === 0) {
                    console.log('⏰ 等待元素生成，延遲重試TouchDragUtility註冊...');
                    setTimeout(registerTouchDrag, 100);
                    return;
                }
                
                console.log(`✅ 找到 ${sourceItems.length} 個可拖拽元素，開始註冊TouchDragUtility`);
                        ...
                        const syntheticDropEvent = {
                            ...
                            dataTransfer: { 
                                getData: () => {
                                    ...
                                    console.log('🎯 [C4-觸控拖拽] 生成拖拽資料:', dropData);
                                    return dropData;
                                }
                            },
        ...
        setupTouchDragForHardMode(question) {
            ...
            const registerTouchDrag = () => {
                ...
                if (sourceItems.length === 0) {
                    console.log('⏰ [困難模式] 等待元素生成，延遲重試TouchDragUtility註冊...');
                    setTimeout(registerTouchDrag, 100);
                    return;
                }
                
                console.log(`✅ [困難模式] 找到 ${sourceItems.length} 個可拖拽元素，開始註冊TouchDragUtility`);
                ...
                if (dropZone) {
                    window.TouchDragUtility.registerDropZone(dropZone, () => true);
                    console.log('✅ [困難模式] 放置區域已註冊');
                }
                if (moneySource) {
                    window.TouchDragUtility.registerDropZone(moneySource, () => true);
                    console.log('✅ [困難模式] 金錢來源區域已註冊');
                }
            };
